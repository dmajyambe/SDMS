...
data = read_data()
@st.cache
def schools_per_school_setting(data):
    
    setting_counts = data['school_settings'].value_counts().reset_index()
    setting_counts.columns = ['School Setting', 'Number of Schools']
    ...
    fig = px.bar(
        ...
        x='School Setting',
        y='Number of Schools',
        title='Number of Schools per School Setting',
    )
...
def plot_unassigned_teachers_by_district(data):
    ...
    # Create a bar chart for total unassigned teachers per district
    plt.figure(figsize=(10, 6))
...
def plot_pupil_teacher_ratios(data):
    """
    Plot a bar graph for pupil-teacher ratios by level.

    Parameters:
    - data: DataFrame with columns 'Level' and 'Pupil-Teacher Ratio'
    """
    plt.figure(figsize=(10, 6))
    ax = sns.barplot(data=data, x='Level', y='Pupil-Teacher Ratio')
...
def plot_water_facilities(data):
    ...
    # Sum the binary values to get the count of schools with each type of water facility
    water_counts = water_df.sum()

    # Plot the count of schools with each water facility
    plt.figure(figsize=(10, 6))
...
def plot_administrative_staff_by_status(data):
    """
    Plots stacked bars showing administrative staff (female and male) per school status.

    Parameters:
    - data: DataFrame containing the relevant columns, including 'school_status', 'administrative_staff_male', and 'administrative_staff_female'.
    """
